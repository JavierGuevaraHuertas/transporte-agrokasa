...
export default function UsuariosPanel({ showToast }: Props) {
  ...
  const cargar = async () => {
    ...
    try {
      ...
      // filtrar solo supervisores (excluir admin)
      const sups = data.filter((u: Supervisor) => u.rol === 'supervisor')
      setSupervisores(sups)
      // cargar áreas de cada supervisor
      const aMap: Record<string, string[]> = {}
      await Promise.all(sups.map(async (s: Supervisor) => {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      setAreas(prev => ({ ...prev, [editing.id]: selected }))
      setEditing(null)
            ...
            <div key={s.id} className="card flex items-start gap-3 hover:border-gray-300">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-1.5 flex-wrap">
                  {sAreas.length > 0 ? (
                    sAreas.map(a => (
                      <span key={a} className="px-2 py-0.5 rounded-full text-xs font-medium bg-green-50 text-green-700 border border-green-200">
                        {a}
                      </span>
                    ))
                  ) : (
                    <span className="text-xs text-amber-500 italic">Sin áreas asignadas — verá todas</span>
                  )}
                </div>
              </div>
              <button
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/45 z-50 flex items-start justify-center pt-16 px-4">
          <div className="bg-white rounded-2xl p-5 max-w-sm w-full shadow-xl">
            <h3 className="text-sm font-bold text-gray-900 mb-1">Editar áreas — {editing.nombre}</h3>
            <p className="text-xs text-gray-500 mb-3">Selecciona las áreas que puede programar este supervisor</p>
              ...
              {ALL_AREAS.map(a => {
                ...
                return (
                  <label
                    key={a}
                    onClick={() => toggle(a)}
                    className={`flex items-center gap-2.5 px-3 py-2 rounded-lg border cursor-pointer transition-colors ${checked ? 'bg-green-50 border-green-200' : 'border-gray-100 hover:bg-gray-50'}`}
                  >
                    <input
                      type="checkbox"
                      checked={checked}
            ...
            <div className="flex gap-2">
              <button onClick={() => setEditing(null)} disabled={saving} className="flex-1 btn-secondary text-xs py-2">
                Cancelar
              </button>
              <button onClick={saveEdit} disabled={saving} className="flex-1 btn-primary text-xs py-2">
