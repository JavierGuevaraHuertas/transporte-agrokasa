...
`
  ...
  const exportarReporteRutas = (filtroTipo: 'SALIDA' | 'RECOJO') => {
    ...
    const tdL = (extra = '') => `padding:3px 5px;font-size:9px;border:1px solid #e5e7eb;text-align:left;vertical-align:middle;white-space:nowrap;${extra}`
    ...
    hors.forEach((hor) => {
      ...
      Object.entries(RUTAS).forEach(([ruta, filas]) => {
        filas.forEach((fila) => {
          // Los datos en m.data usan ruta con '-' reemplazado por '_'
          const rutaTxt = ruta.replace('-', '_')
          const ridData = fila.lbl ? `${rutaTxt}_${fila.lbl}` : `${rutaTxt}_${fila.l ?? 0}_${fila.c ?? 0}`
          const rid = getRid(ruta, fila) // para mostrar en tabla
          const vals: Record<string, number> = {}
          let rowTotal = 0
          areasList.forEach((a) => {
            let s = 0
            progsHor.filter((m) => m.area === a).forEach((m) => { Object.keys(m.data).forEach((ck) => { if (ck.startsWith(ridData + '||')) s += m.data[ck] || 0 }) })
            vals[a] = s; rowTotal += s
          })
          if (rowTotal > 0) horFilas[hor].push({ ruta, lote: fila.lbl ? '' : String(fila.l ?? ''), com: fila.lbl ? '' : String(fila.c ?? ''), lbl: fila.lbl ?? '', rid, vals, rowTotal })
        })
      })
    })
    ...
    areasList.forEach((a) => { totArea[a] = 0 })
    let grandTotal = 0
    Object.values(horFilas).forEach((filas) => { filas.forEach((f) => { areasList.forEach((a) => { totArea[a] += f.vals[a] || 0 }); grandTotal += f.rowTotal }) })
    ...
    const bodyRows = hors.map((hor) => {
      ...
      return filas.map((fila) => {
        const isFirstHor = !horRendered; if (isFirstHor) horRendered = true
        const isFirstRuta = !rutaRendered.has(fila.ruta); if (isFirstRuta) rutaRendered.add(fila.ruta)
        const horCell = isFirstHor ? `<td rowspan="${horSpan}" style="${tdL('width:1%;white-space:nowrap;font-weight:700;color:#155724;background:#d4edda;vertical-align:middle;text-align:center;')}">${hor}</td>` : ''
        const rutaCell = isFirstRuta ? `<td rowspan="${rutaSpan[fila.ruta]}" style="${td('width:1%;white-space:nowrap;font-weight:700;color:#2563eb;vertical-align:middle;background:#eff6ff;')}">${fila.ruta}</td>` : ''
        const loteComCells = fila.lbl ? `<td colspan="2" style="${td('width:1%;white-space:nowrap;color:#6b7280;font-style:italic;text-align:left;')}">${fila.lbl}</td>` : `<td style="${td('width:1%;white-space:nowrap;')}">${fila.lote}</td><td style="${td('width:1%;white-space:nowrap;')}">${fila.com}</td>`
        const totalCell = `<td style="${td('width:1%;white-space:nowrap;font-weight:700;color:#059669;background:#f0fdf4;')}">${fila.rowTotal || ''}</td>`
        const areaCells = areasList.map((a, i) => { const v = fila.vals[a] || 0; const bg = i % 2 === 0 ? '' : 'background:#f9fafb;'; return `<td style="${td('width:26px;max-width:26px;' + bg + (v ? 'font-weight:600;' : 'color:#d1d5db;'))}">${v || ''}</td>` }).join('')
    ...
    <style>
      ...
      <div id="reporte">
        ...
        <table style="border-collapse:collapse;font-size:9px;width:auto">
          <thead>
            <tr>
              <th style="${th('min-width:70px;vertical-align:middle;')}">HORARIO</th>
              <th style="${th('min-width:36px;vertical-align:middle;')}">RUTA</th>
              <th style="${th('min-width:28px;vertical-align:middle;')}">LOTE</th>
              <th style="${th('min-width:28px;vertical-align:middle;')}">COM</th>
              <th style="${th('min-width:36px;vertical-align:middle;')}">TOTAL</th>
              ${areaHeaders}
            </tr>
          </thead>
          <tbody>${bodyRows}${totalRow}</tbody>
  ...
  const exportarReporteComedores = (filtroTipo: 'SALIDA' | 'RECOJO') => {
    ...
    // Encabezado de agrupadores + paraderos (doble fila)
    const agHeaders = AGK.map((ag) =>
      `<th colspan="${AGR[ag].length}" style="${th('border-bottom:1px solid #0f4f28;font-size:10px;background:#155e30;')}">${ag}</th>`
    ).join('')

    const parHeaders = ALLP.map(({ ag, p }, i) => {
      const bl = i > 0 && ALLP[i - 1].ag !== ag ? 'border-left:2px solid #0f4f28;' : ''
      return `<th style="${th(bl + 'width:28px;min-width:28px;padding:0;font-size:9px;')}">
        <div style="height:90px;display:flex;align-items:center;justify-content:center;">
          <span style="writing-mode:vertical-rl;transform:rotate(180deg);font-size:9px;line-height:1.3;text-align:center;">${p}</span>
        </div>
      </th>`
    }).join('')

    // Construir filas por horario > ruta > lote/comedor
    const bodyRows = hors.map((hor) => {
      const progsHor = allFiltrado.filter((m) => m.hor === hor)

      type FilaRutaVis = { ruta: string; lote: string; com: string; lbl: string; pars: Record<string, number>; rowTotal: number }
      const filas: FilaRutaVis[] = []
      ...
      return filas.map((fila) => {
        ...
        const horCell = isFirstHor
          ? `<td rowspan="${filas.length}" style="${tdL('font-weight:700;color:#155724;background:#d4edda;text-align:center;border-top:2px solid #6ee7b7;')}">${hor}</td>` : ''
        const rutaCell = isFirstRuta
          ? `<td rowspan="${rutaSpan[fila.ruta]}" style="${td('font-weight:700;color:#2563eb;background:#eff6ff;' + (isFirstHor ? 'border-top:2px solid #6ee7b7;' : ''))}">${fila.ruta}</td>` : ''
        const borderTop = isFirstHor ? 'border-top:2px solid #6ee7b7;' : ''
        const loteComCells = fila.lbl
          ? `<td colspan="2" style="${td('color:#6b7280;font-style:italic;text-align:left;' + borderTop)}">${fila.lbl}</td>`
          : `<td style="${td(borderTop)}">${fila.lote}</td><td style="${td(borderTop)}">${fila.com}</td>`
        const parCells = ALLP.map(({ ag, p }, i) => {
          const v = fila.pars[p] || 0
          const bl = i > 0 && ALLP[i - 1].ag !== ag ? 'border-left:2px solid #d1fae5;' : ''
          return `<td style="${td(bl + borderTop + (v ? 'font-weight:600;' : 'color:#d1d5db;'))}">${v || ''}</td>`
        }).join('')
        const totalCell = `<td style="${td('font-weight:700;color:#059669;background:#f0fdf4;border-left:2px solid #d1fae5;' + borderTop)}">${fila.rowTotal}</td>`

        return `<tr>${horCell}${rutaCell}${loteComCells}${parCells}${totalCell}</tr>`
      }).join('')
    }).join('')
    ...
    const totalRow = `<tr style="background:#bbf7d0">
      ...
      ${ALLP.map(({ ag, p }, i) => {
        const v = totPar[p] || 0
        const bl = i > 0 && ALLP[i - 1].ag !== ag ? 'border-left:2px solid #6ee7b7;' : ''
        return `<td style="${td('font-weight:700;color:#14532d;' + bl)}">${v || ''}</td>`
      }).join('')}
      <td style="${td('font-weight:800;color:#14532d;border-left:2px solid #6ee7b7;')}">${grandTotal}</td>
    </tr>`
    ...
    <style>
      ...
      <div id="reporte">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
          <div>
            <h2 style="margin:0 0 4px;font-size:13px;font-weight:800">Distribución de personal por comedor</h2>
            <span style="font-size:11px;font-weight:700;color:${color};padding:2px 10px;background:${colorLight};border-radius:4px;border:1px solid ${color}">${tipoLabel}</span>
          </div>
          <span style="font-size:10px;color:#555">Fecha: ${fecha}</span>
        </div>
        <table style="border-collapse:collapse;font-size:9px;width:auto">
          <thead>
            <tr>
              <th style="${th('min-width:70px;')}" rowspan="2">HORARIO</th>
              <th style="${th('min-width:36px;')}" rowspan="2">RUTA</th>
              <th style="${th('min-width:28px;')}" rowspan="2">LOTE</th>
              <th style="${th('min-width:28px;')}" rowspan="2">COM</th>
              ${agHeaders}
              <th style="${th('min-width:36px;border-left:2px solid #6ee7b7;')}" rowspan="2">TOTAL</th>
            </tr>
            <tr>${parHeaders}</tr>
          </thead>
          <tbody>${bodyRows}${totalRow}</tbody>
  ...
  const exportarExcelRutas = (filtroTipo: 'SALIDA' | 'RECOJO') => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const thS = (e?: any): any => ({ font: { bold: true, color: { rgb: WHITE }, sz: 9 }, fill: { fgColor: { rgb: GREEN_DARK } }, border: { top: { style: 'thin', color: { rgb: '155e30' } }, bottom: { style: 'thin', color: { rgb: '155e30' } }, left: { style: 'thin', color: { rgb: '155e30' } }, right: { style: 'thin', color: { rgb: '155e30' } } }, alignment: { horizontal: 'center', vertical: 'center', wrapText: true }, ...e })
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const tdS = (e?: any): any => ({ font: { sz: 9 }, border: { top: { style: 'thin', color: { rgb: 'e5e7eb' } }, bottom: { style: 'thin', color: { rgb: 'e5e7eb' } }, left: { style: 'thin', color: { rgb: 'e5e7eb' } }, right: { style: 'thin', color: { rgb: 'e5e7eb' } } }, alignment: { horizontal: 'center', vertical: 'center' }, ...e })
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const tdLS = (e?: any): any => ({ ...tdS(), alignment: { horizontal: 'left', vertical: 'center' }, ...e })
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    let r = 0
    sc(r, 0, `Distribución por ruta — ${tipoLabel}`, { font: { bold: true, sz: 13 }, alignment: { horizontal: 'left' } }); addMerge(r, 0, r, 5); r++
    sc(r, 0, `Fecha: ${fecha}`, { font: { sz: 9, color: { rgb: '555555' } } }); addMerge(r, 0, r, 5); r++; r++

    let c = 0
    ;['HORARIO', 'RUTA', 'LOTE', 'COM', 'TOTAL'].forEach((h) => { sc(r, c++, h, thS()) })
    areasList.forEach((a) => { sc(r, c++, a, thS({ alignment: { horizontal: 'center', vertical: 'bottom', textRotation: 90, wrapText: true } })) })
    r++

    hors.forEach((hor) => {
      const progsHor = allFiltrado.filter((m) => m.hor === hor)
      type FilaVis = { ruta: string; lote: string; com: string; lbl: string; vals: Record<string, number>; rowTotal: number }
      const filas: FilaVis[] = []
      Object.entries(RUTAS).forEach(([ruta, rutaFilas]) => {
        rutaFilas.forEach((fila) => {
          const rutaTxt = ruta.replace('-', '_')
          const ridData = fila.lbl ? `${rutaTxt}_${fila.lbl}` : `${rutaTxt}_${fila.l ?? 0}_${fila.c ?? 0}`
          const vals: Record<string, number> = {}; let rowTotal = 0
          areasList.forEach((a) => { let s = 0; progsHor.filter((m) => m.area === a).forEach((m) => { Object.keys(m.data).forEach((ck) => { if (ck.startsWith(ridData + '||')) s += m.data[ck] || 0 }) }); vals[a] = s; rowTotal += s })
          if (rowTotal > 0) filas.push({ ruta, lote: fila.lbl ? '' : String(fila.l ?? ''), com: fila.lbl ? '' : String(fila.c ?? ''), lbl: fila.lbl ?? '', vals, rowTotal })
        })
      })
      if (!filas.length) return
      ...
      filas.forEach((fila, fi) => {
        c = 0
        if (fi === 0) { sc(r, c, hor, tdS({ fill: { fgColor: { rgb: GREEN_LIGHT } }, font: { bold: true, sz: 9, color: { rgb: '155724' } }, alignment: { horizontal: 'center', vertical: 'center' } })); if (filas.length > 1) addMerge(startR, 0, startR + filas.length - 1, 0) }
        c = 1
        if (!rutaStartRow[fila.ruta]) { rutaStartRow[fila.ruta] = r; sc(r, c, fila.ruta, tdS({ fill: { fgColor: { rgb: BLUE_PALE } }, font: { bold: true, color: { rgb: '2563eb' }, sz: 9 }, alignment: { horizontal: 'center', vertical: 'center' } })); if (rutaSpan[fila.ruta] > 1) addMerge(r, 1, r + rutaSpan[fila.ruta] - 1, 1) }
        c = 2
        if (fila.lbl) { sc(r, c++, fila.lbl, tdLS({ font: { italic: true, color: { rgb: '6b7280' }, sz: 9 } })); sc(r, c++, '', tdS()) } else { sc(r, c++, fila.lote, tdS()); sc(r, c++, fila.com, tdS()) }
        sc(r, c++, fila.rowTotal || '', tdS({ font: { bold: true, color: { rgb: '059669' }, sz: 9 }, fill: { fgColor: { rgb: GREEN_PALE } } }))
        areasList.forEach((a, i) => { const v = fila.vals[a] || 0; const bg = i % 2 === 0 ? WHITE : 'f9fafb'; sc(r, c++, v || '', v ? tdS({ font: { bold: true, sz: 9 }, fill: { fgColor: { rgb: bg } } }) : tdS({ font: { color: { rgb: 'd1d5db' }, sz: 9 }, fill: { fgColor: { rgb: bg } } })) })
        r++
      })
    })

    const totArea: Record<string, number> = {}; areasList.forEach((a) => { totArea[a] = 0 }); let grandTotal = 0
    hors.forEach((hor) => { const progsHor = allFiltrado.filter((m) => m.hor === hor); Object.entries(RUTAS).forEach(([ruta, rutaFilas]) => { rutaFilas.forEach((fila) => { const rutaTxt = ruta.replace('-', '_'); const ridData = fila.lbl ? `${rutaTxt}_${fila.lbl}` : `${rutaTxt}_${fila.l ?? 0}_${fila.c ?? 0}`; areasList.forEach((a) => { progsHor.filter((m) => m.area === a).forEach((m) => { Object.keys(m.data).forEach((ck) => { if (ck.startsWith(ridData + '||')) { totArea[a] = (totArea[a] || 0) + (m.data[ck] || 0); grandTotal += m.data[ck] || 0 } }) }) }) }) }) })

    c = 0
    sc(r, c, 'TOTAL', tdLS({ font: { bold: true, sz: 9, color: { rgb: '14532d' } }, fill: { fgColor: { rgb: GREEN_MID } } })); addMerge(r, 0, r, 3); c = 4
    sc(r, c++, grandTotal, tdS({ font: { bold: true, sz: 9, color: { rgb: '14532d' } }, fill: { fgColor: { rgb: GREEN_MID } } }))
    areasList.forEach((a, i) => { const v = totArea[a] || 0; const bg = i % 2 === 0 ? GREEN_MID : 'a7f3d0'; sc(r, c++, v || '', tdS({ font: { bold: true, color: { rgb: '14532d' }, sz: 9 }, fill: { fgColor: { rgb: bg } } })) })

    ws['!cols'] = [{ wch: 14 }, { wch: 8 }, { wch: 6 }, { wch: 6 }, { wch: 7 }, ...areasList.map(() => ({ wch: 14 }))]
    ws['!merges'] = merges
    ws['!ref'] = XLSX.utils.encode_range({ s: { r: 0, c: 0 }, e: { r, c: 4 + areasList.length } })
    XLSX.utils.book_append_sheet(wb, ws, 'Por Ruta')
  ...
  const exportarExcelComedores = (filtroTipo: 'SALIDA' | 'RECOJO') => {
    ...
    let r = 0
    sc(r, 0, `Distribución por comedor — ${tipoLabel}`, { font: { bold: true, sz: 13 }, alignment: { horizontal: 'left' } }); addMerge(r, 0, r, 5); r++
    sc(r, 0, `Fecha: ${fecha}`, { font: { sz: 9, color: { rgb: '555555' } } }); addMerge(r, 0, r, 5); r++; r++

    // Encabezado doble: agrupadores + paraderos
    let c = 0
    ;['HORARIO', 'RUTA', 'LOTE', 'COM', 'TOTAL'].forEach((h) => { sc(r, c, h, thS()); addMerge(r, c, r + 1, c); c++ })
    AGK.forEach((ag) => { sc(r, c, ag, thS()); addMerge(r, c, r, c + AGR[ag].length - 1); c += AGR[ag].length })
    r++
    c = 5
    ALLP.forEach(({ p }) => { sc(r, c++, p, thS({ alignment: { horizontal: 'center', vertical: 'bottom', textRotation: 90, wrapText: true } })) })
    r++

    hors.forEach((hor) => {
      const progsHor = allFiltrado.filter((m) => m.hor === hor)
      type FilaVis = { ruta: string; lote: string; com: string; lbl: string; pars: Record<string, number>; rowTotal: number }
      const filas: FilaVis[] = []
      Object.entries(RUTAS).forEach(([ruta, rutaFilas]) => {
        rutaFilas.forEach((fila) => {
          const rutaTxt = ruta.replace('-', '_')
          const ridData = fila.lbl ? `${rutaTxt}_${fila.lbl}` : `${rutaTxt}_${fila.l ?? 0}_${fila.c ?? 0}`
          const pars: Record<string, number> = {}; let rowTotal = 0
          ALLP.forEach(({ p }) => { let s = 0; progsHor.forEach((m) => { Object.keys(m.data).forEach((ck) => { if (ck.startsWith(ridData + '||') && ck.endsWith('||' + p)) s += m.data[ck] || 0 }) }); pars[p] = s; rowTotal += s })
          if (rowTotal > 0) filas.push({ ruta, lote: fila.lbl ? '' : String(fila.l ?? ''), com: fila.lbl ? '' : String(fila.c ?? ''), lbl: fila.lbl ?? '', pars, rowTotal })
        })
      })
      if (!filas.length) return
      ...
      filas.forEach((fila, fi) => {
        ...
        if (fila.lbl) { sc(r, c++, fila.lbl, tdLS({ font: { italic: true, color: { rgb: '6b7280' }, sz: 9 } })); sc(r, c++, '', tdS()) } else { sc(r, c++, fila.lote, tdS()); sc(r, c++, fila.com, tdS()) }
        sc(r, c++, fila.rowTotal || '', tdS({ font: { bold: true, color: { rgb: '059669' }, sz: 9 }, fill: { fgColor: { rgb: GREEN_PALE } } }))
        ALLP.forEach(({ p }) => { const v = fila.pars[p] || 0; sc(r, c++, v || '', v ? tdS({ font: { bold: true, sz: 9 } }) : tdS({ font: { color: { rgb: 'd1d5db' }, sz: 9 } })) })
        r++
      })
    })

    const totPar: Record<string, number> = {}; ALLP.forEach(({ p }) => { totPar[p] = 0 }); let grandTotal = 0
    allFiltrado.forEach((m) => { ALLP.forEach(({ p }) => { Object.keys(m.data).forEach((ck) => { if (ck.endsWith('||' + p)) totPar[p] += m.data[ck] || 0 }) }); grandTotal += m.total || 0 })

    c = 0
    sc(r, c, 'TOTAL', tdLS({ font: { bold: true, sz: 9, color: { rgb: '14532d' } }, fill: { fgColor: { rgb: GREEN_MID } } })); addMerge(r, 0, r, 3); c = 4
    sc(r, c++, grandTotal, tdS({ font: { bold: true, sz: 9, color: { rgb: '14532d' } }, fill: { fgColor: { rgb: GREEN_MID } } }))
    ALLP.forEach(({ p }) => { const v = totPar[p] || 0; sc(r, c++, v || '', tdS({ font: { bold: true, color: { rgb: '14532d' }, sz: 9 }, fill: { fgColor: { rgb: GREEN_MID } } })) })

    ws['!cols'] = [{ wch: 14 }, { wch: 8 }, { wch: 6 }, { wch: 6 }, { wch: 7 }, ...Array(ALLP.length).fill(null).map(() => ({ wch: 5 }))]
    ws['!merges'] = merges
    ws['!ref'] = XLSX.utils.encode_range({ s: { r: 0, c: 0 }, e: { r, c: 4 + ALLP.length } })
    XLSX.utils.book_append_sheet(wb, ws, 'Por Comedor')
