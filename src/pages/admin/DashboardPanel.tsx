...
export default function DashboardPanel({ refresh, onDiaChange, showToast }: Props) {
  ...
  const exportarExcel = (filtroTipo: 'SALIDA' | 'RECOJO') => {
    ...
    c = 0
    sc(r, c, 'HORARIO', thStyle()); addMerge(r, c, r + 1, c); c++
    sc(r, c, 'TOTAL', thStyle()); addMerge(r, c, r + 1, c); c++
    AGK.forEach((ag) => { sc(r, c, ag, thStyle()); addMerge(r, c, r, c + AGR[ag].length - 1); c += AGR[ag].length })
    r++

    c = 2
    ALLP.forEach(({ p }) => { sc(r, c++, p, thStyle({ alignment: { horizontal: 'center', vertical: 'bottom', textRotation: 90, wrapText: true } })) })
    r++
    ...
    Object.entries(grupos).forEach(([_hor, filas]) => {
      ...
      ALLP.forEach(({ p }) => { const v = horPar[p] || 0; sc(r, c++, v || '', v ? tdStyle({ font: { bold: true, color: { rgb: '065f46' }, sz: 9 }, fill: { fgColor: { rgb: GREEN_PALE } } }) : tdStyle({ font: { color: { rgb: 'd1d5db' }, sz: 9 }, fill: { fgColor: { rgb: GREEN_PALE } } })) })
      r++
    })
    ...
    c = 0
    ;['HORARIO', 'RUTA', 'LOTE', 'COM', 'TOTAL'].forEach((h) => { sc(r, c++, h, thStyle()) })
    areasList.forEach((a) => { sc(r, c++, a, thStyle({ alignment: { horizontal: 'center', vertical: 'bottom', textRotation: 90, wrapText: true } })) })
    r++
    ...
    hors.forEach((hor) => {
      ...
      filas.forEach((fila, fi) => {
        ...
        c = 2
        if (fila.lbl) { sc(r, c++, fila.lbl, tdLStyle({ font: { italic: true, color: { rgb: '6b7280' }, sz: 9 } })); sc(r, c++, '', tdStyle()) }
        else { sc(r, c++, fila.lote, tdStyle()); sc(r, c++, fila.com, tdStyle()) }
        sc(r, c++, fila.rowTotal || '', tdStyle({ font: { bold: true, color: { rgb: '059669' }, sz: 9 }, fill: { fgColor: { rgb: GREEN_PALE } } }))
        areasList.forEach((a, i) => { const v = fila.vals[a] || 0; const bg = i % 2 === 0 ? WHITE : 'f9fafb'; sc(r, c++, v || '', v ? tdStyle({ font: { bold: true, sz: 9 }, fill: { fgColor: { rgb: bg } } }) : tdStyle({ font: { color: { rgb: 'd1d5db' }, sz: 9 }, fill: { fgColor: { rgb: bg } } })) })
        r++
      })
    })
    ...
    c = 0
    sc(r, c, 'TOTAL', tdLStyle({ font: { bold: true, sz: 9, color: { rgb: '14532d' } }, fill: { fgColor: { rgb: GREEN_MID } } }))
    addMerge(r, 0, r, 3); c = 4
    sc(r, c++, grandTotalRuta, tdStyle({ font: { bold: true, sz: 9, color: { rgb: '14532d' } }, fill: { fgColor: { rgb: GREEN_MID } } }))
    areasList.forEach((a, i) => { const v = totArea[a] || 0; const bg = i % 2 === 0 ? GREEN_MID : 'a7f3d0'; sc(r, c++, v || '', tdStyle({ font: { bold: true, color: { rgb: '14532d' }, sz: 9 }, fill: { fgColor: { rgb: bg } } })) })
    r += 2
    ...
    hors.forEach((hor) => {
      ...
      sc(r, c++, horTotal, tdStyle({ font: { bold: true, sz: 9, color: { rgb: '065f46' } }, fill: { fgColor: { rgb: GREEN_PALE } } }))
      areasList.forEach((a, i) => { const v = horArea[a] || 0; const bg = i % 2 === 0 ? GREEN_PALE : 'e8faf0'; sc(r, c++, v || '', v ? tdStyle({ font: { bold: true, color: { rgb: '065f46' }, sz: 9 }, fill: { fgColor: { rgb: bg } } }) : tdStyle({ font: { color: { rgb: 'd1d5db' }, sz: 9 }, fill: { fgColor: { rgb: bg } } })) })
      r++
    })
    ...
    ws['!cols'] = [{ wch: 14 }, { wch: 20 }, { wch: 22 }, { wch: 6 }, { wch: 7 }, ...Array(Math.max(ALLP.length, areasList.length)).fill(null).map(() => ({ wch: 5 }))]
    ws['!merges'] = merges
    ws['!ref'] = XLSX.utils.encode_range({ s: { r: 0, c: 0 }, e: { r, c: 5 + Math.max(ALLP.length, areasList.length) } })
  ...
  const exportarReporteRutas = (filtroTipo: 'SALIDA' | 'RECOJO') => {
    ...
    hors.forEach((hor) => {
      ...
      Object.entries(RUTAS).forEach(([ruta, filas]) => {
        filas.forEach((fila) => {
          ...
          areasList.forEach((a) => {
            let s = 0
            progsHor.filter((m) => m.area === a).forEach((m) => { Object.keys(m.data).forEach((ck) => { if (ck.startsWith(rid + '||')) s += m.data[ck] || 0 }) })
            vals[a] = s; rowTotal += s
          })
    ...
    const areaHeaders = areasList.map((a, i) => `<th style="${th('width:26px;max-width:26px;height:110px;padding:0;vertical-align:middle;text-align:center;' + (i > 0 ? 'border-left:1px solid #155e30;' : ''))}"><div style="display:table;width:26px;height:110px;"><div style="display:table-cell;vertical-align:middle;text-align:center;"><span style="writing-mode:vertical-rl;transform:rotate(180deg);font-size:9px;line-height:1.2;word-break:break-word;white-space:normal;width:14px;display:inline-block;text-align:center;">${a}</span></div></div></th>`).join('')

    const html = `<html><head><meta charset="utf-8">
    <style>body{font-family:Arial,sans-serif;font-size:9px;margin:16px;background:white}@media print{body{margin:8px} @page{size:landscape;margin:10mm}}</style></head><body>
      <div id="reporte">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
          <div>
        ...
        <table style="border-collapse:collapse;font-size:9px;table-layout:fixed">
          <colgroup><col style="width:70px"><col style="width:36px"><col style="width:28px"><col style="width:28px"><col style="width:36px">${areasList.map(() => `<col style="width:26px">`).join('')}</colgroup>
          <thead><tr><th style="${th('vertical-align:middle;')}">HORARIO</th><th style="${th('vertical-align:middle;')}">RUTA</th><th style="${th('vertical-align:middle;')}">LOTE</th><th style="${th('vertical-align:middle;')}">COM</th><th style="${th('vertical-align:middle;')}">TOTAL</th>${areaHeaders}</tr></thead>
          <tbody>${bodyRows}${totalRow}</tbody>
        </table>
        <div style="margin-top:20px">
          <p style="font-size:9px;font-weight:700;color:#14532d;margin-bottom:6px">RESUMEN POR HORARIO — ${tipoLabel}: ${grandTotal} personas</p>
          <table style="border-collapse:collapse;font-size:9px;table-layout:fixed">
            <colgroup><col style="width:70px"><col style="width:36px">${areasList.map(() => `<col style="width:26px">`).join('')}</colgroup>
            <thead><tr><th style="${th()}">HORARIO</th><th style="${th()}">TOTAL</th>${areaHeaders}</tr></thead>
            <tbody>${subRows}${subTotalRow}</tbody>
          </table>
        </div>
      </div>
    ...
    return (
      <div className={`card border-2 ${isSal ? 'border-amber-100' : 'border-blue-100'}`}>
        ...
        <div className={`flex gap-2 mt-3 pt-2 border-t ${isSal ? 'border-amber-100' : 'border-blue-100'}`}>
          ...
          <button onClick={() => exportarExcel(tipo)} className={`flex-1 text-xs font-semibold py-1.5 rounded-lg text-white ${isSal ? 'bg-amber-600 hover:bg-amber-700' : 'bg-blue-700 hover:bg-blue-800'}`}>
            📥 XLS
          </button>
        </div>
      </div>
    ...
    <div>
      <div className="card mb-3 flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            onClick={() => {
              const csvData = all.map((x) => ({ key: x.id, user: x.user, tipo: x.tipo, hor: x.hor, area: x.area, total: x.total, data: x.data })) as unknown as ProgramacionWithData[]
              exportToCSV(csvData)
            }}
