...
export default function TendenciasPanel({ refresh: _r }: Props) {
  const [tipo, setTipo] = useState('ALL')
  const [desde, setDesde] = useState(() => { const d = new Date(); d.setDate(d.getDate() - 28); return d.toISOString().slice(0, 10) })
  ...
  const setSem = (n: number) => {
    const d = new Date(); const from = new Date(); from.setDate(d.getDate() - n * 7)
    setDesde(from.toISOString().slice(0, 10)); setHasta(d.toISOString().slice(0, 10))
  }

  const toggleArea = (a: string) => setVisible(prev => ({ ...prev, [a]: prev[a] === false ? true : false }))

  useEffect(() => {
    const all = getAllProgs()
    const filtered = all.filter(m => {
      if (tipo !== 'ALL' && m.tipo !== tipo) return false
      const dk = new Date(m.ts).toISOString().slice(0, 10)
      return dk >= desde && dk <= hasta
    })

    const areas = [...new Set(filtered.map(x => x.area))].sort()
    areas.forEach(a => { if (visible[a] === undefined) setVisible(prev => ({ ...prev, [a]: true })) })

    const weeks = [...new Set(filtered.map(m => getWeekKey(m.ts)))].sort()
    const byAW: Record<string, Record<string, number>> = {}
    areas.forEach(a => { byAW[a] = {}; weeks.forEach(w => { byAW[a][w] = 0 }) })
    filtered.forEach(m => { const wk = getWeekKey(m.ts); if (byAW[m.area]) byAW[m.area][wk] = (byAW[m.area][wk] || 0) + (m.total || 0) })
  ...
  }, [tipo, desde, hasta, visible])

  const all = getAllProgs()
  const filtered = all.filter(m => { if (tipo !== 'ALL' && m.tipo !== tipo) return false; const dk = new Date(m.ts).toISOString().slice(0, 10); return dk >= desde && dk <= hasta })
  const areas = [...new Set(filtered.map(x => x.area))].sort()
        ...
        <div className="flex gap-3 flex-wrap items-end">
          <div>
            ...
            <select value={tipo} onChange={e => setTipo(e.target.value)} className="input-base text-xs w-auto">
              <option value="ALL">Salida + Recojo</option>
              <option value="SALIDA">Solo Salida</option>
              <option value="RECOJO">Solo Recojo</option>
            </select>
          </div>
          <div>
