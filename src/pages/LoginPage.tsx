 ...
 * El email se construye como: username@agrokasa.com.pe
 * La contraseña es la que se configure en Supabase Auth.
 *
 * Para usar este archivo, renómbralo a LoginPage.tsx
 * y elimina el LoginPage.tsx original.
 */
export default function LoginPageSupabase() {
  ...
  useEffect(() => {
    ...
    if (saved) { setUsername(saved); setRememberState(true) }
  }, [])

  const handleLogin = async () => {
    const u = username.trim().toLowerCase()
    if (!u || !password) { setError('Completa usuario y contraseña'); return }
    setLoading(true); setError('')
    try {
      await signIn(`${u}@agrokasa.com.pe`, password)
      if (remember) setRemember(u)
      // useAuth en App.tsx detectará el cambio de sesión automáticamente
    } catch (e: unknown) {
    ...
    <div className="min-h-screen bg-[#eef1f5] flex items-center justify-center p-5">
      <div className="w-full max-w-sm rounded-2xl overflow-hidden shadow-xl">
        <div className="bg-primary-600 px-7 py-7 text-white">
          <div className="w-11 h-11 bg-white/20 rounded-xl flex items-center justify-center font-bold text-sm mb-3">PT</div>
        ...
        <div className="bg-white px-7 py-6">
          ...
          <label className="flex items-center gap-2 text-xs text-gray-500 mb-4 cursor-pointer">
            <input type="checkbox" checked={remember} onChange={e => setRememberState(e.target.checked)} className="w-3.5 h-3.5 accent-primary-600" />
            Recordar usuario
          </label>
